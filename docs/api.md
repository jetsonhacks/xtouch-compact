...
| `send(message)` | Raw output bypassing deduplication and touch. Successful matching output invalidates non-fader history; motor output records its value and invalidates observation currency. See [Raw Diagnostic Output](usage.md#raw-diagnostic-output). |
