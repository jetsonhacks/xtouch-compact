...
## Observation Disposition Register

For each significant finding below, this records what the project decided to
do about it, not just what was seen. Disposition categories:

- `implemented` — the finding is encoded in current behavior.
- `deliberately application-owned` — the library exposes the underlying
  state but leaves the policy decision to the caller.
- `unsupported` — the capability is not exposed by this library.
- `unresolved` — the underlying cause or value is not yet known.

| Observation | Disposition | Reason | Implementation | Test |
|---|---|---|---|---|
| [Fader touch overrides motor; no reliable position echo from ordinary travel](#observed-2026-08-09-representative-faders) | `implemented` | Touch must own the motor while held; host-driven travel cannot be trusted to echo a position. | [`fader_state.py`](../src/xtouch_compact/fader_state.py) | `test_required_ownership_and_reconciliation_scenario` in [`test_fader_state.py`](../tests/test_fader_state.py) |
| [Fader observation validity vs. command history](#observed-2026-08-09-representative-faders) | `implemented` | A newer physical observation must take precedence over a stale desired/commanded value when deciding whether to resend. | [`fader_state.py`](../src/xtouch_compact/fader_state.py) | `test_newer_current_observation_takes_precedence_over_command_history`, `test_observation_validity_after_send_observation_and_reset` in [`test_fader_state.py`](../tests/test_fader_state.py) |
| [Transport-button press replaces host LED state within its group](#observed-2026-08-11-transport-button-press-replaces-host-led-state) | `implemented` | The host has no visible event for the local LED replacement, so its own last-sent record for the group can no longer be trusted after any press. | [`surface_state.py`](../src/xtouch_compact/surface_state.py) (`SurfaceStateController.physical_event`, `_TRANSPORT_BUTTONS` branch) | `test_transport_button_press_invalidates_group_for_led_reassertion` in [`test_surface_state.py`](../tests/test_surface_state.py) |
| [Releasing an assignable button after a host BLINK command can replace, not restore, the last remote LED state](#observed-2026-08-09-buttons-and-layers) | `implemented` | Local release changed the LED without a host-visible event; this is applied as a conservative policy across all assignable buttons based on one representative measurement, not independent characterization of every button. | [`surface_state.py`](../src/xtouch_compact/surface_state.py) (`_invalidate_button_led_on_release`) | `test_release_after_command_and_setter_reasserts`, `test_release_invalidation_does_not_affect_unrelated_button` in [`test_surface_state.py`](../tests/test_surface_state.py) |
| [Encoder rotation is absolute 0–127, accelerated, and repeats at saturation](#observed-2026-08-09-encoders-and-rings) | `implemented` | Raw repeated boundary values and acceleration are real device behavior the decoder must preserve, not noise to filter. | [`decoder.py`](../src/xtouch_compact/decoder.py) | `test_repeated_encoder_values_remain_observable` in [`test_decoder.py`](../tests/test_decoder.py) |
| [Ring mode change and physical rotation both redraw the ring locally](#observed-2026-08-10-ring-redraw-also-follows-physical-rotation-not-only-mode-change) | `implemented` | A locally redrawn ring makes the session's last-sent display record stale; without invalidation, a later identical remote value is wrongly suppressed as a duplicate. | [`surface_state.py`](../src/xtouch_compact/surface_state.py) (mode-change restore, `invalidate_encoder_display`) | `test_encoder_mode_change_restores_known_desired_display`, `test_physical_rotation_invalidates_last_sent_display_for_reassertion` in [`test_surface_state.py`](../tests/test_surface_state.py) |
| [Physical Layer A/B selection has no host-visible event; layer LED alone is not proof of transmit state](#observed-2026-08-09-buttons-and-layers) | `implemented` | The host cannot infer the device's actual selected layer, so `select_layer()` must always transmit and startup must explicitly assert a layer before publishing input. | [`session.py`](../src/xtouch_compact/session.py) (`initialize`, `select_layer`), [`surface_state.py`](../src/xtouch_compact/surface_state.py) | `test_layer_updates_track_state_and_reassert_on_repeated_selection`, `test_startup_layer_assertion_is_recorded_and_never_deduplicated` in [`test_surface_state.py`](../tests/test_surface_state.py); `test_post_connect_layer_initialization_is_explicit_session_policy` in [`test_session.py`](../tests/test_session.py) |
| [Not every incoming message decodes to a typed event](#reported-2026-08-09-alsa-sequencer-enablement) | `implemented` | Unknown addresses and unsupported ALSA event types must not raise or be silently coerced into a wrong typed event; `None` is the correct result. | [`decoder.py`](../src/xtouch_compact/decoder.py), [`alsa_transport.py`](../src/xtouch_compact/alsa_transport.py) (`midi_from_alsa_event`) | `test_decoder_returns_none_for_unknown_or_unsupported_traffic` in [`test_decoder.py`](../tests/test_decoder.py); `test_decoder_none_does_not_fail_receive_pipeline` in [`test_session.py`](../tests/test_session.py) |
| [Foot-control input (expression pedal, foot switch) traffic and ranges](#open-characterization-items) | `unsupported` | The guide's RX description is incomplete and pedal ranges were never measured; only the foot-switch status LED (host-to-device) is implemented. | none (no typed event) | none — intentionally unsupported |
| [Passive receive timeout after USB disconnect/power-off does not report connection loss](#observed-2026-08-09-reconnect-and-power-cycle) | `deliberately application-owned` | A receive timeout only means no supported event arrived; it is not a liveness signal. Explicit `reconnect()` after some other confirmation of device return is the supported recovery path. | [`session.py`](../src/xtouch_compact/session.py) (`reconnect`) | `test_reconnect_uses_new_endpoint_initializes_then_restores_feedback` in [`test_runtime.py`](../tests/test_runtime.py) |
| [Fixed factory Standard MIDI profile replaces runtime YAML/spec loading](#reported-2026-08-08-jetson-agx-thor-characterization) | `implemented` | One fixed, validated factory map is sufficient for the supported hardware and mode; a runtime-loadable specification was removed as an unneeded second source of truth. | [`device_map.py`](../src/xtouch_compact/device_map.py) | [`test_device_map.py`](../tests/test_device_map.py) |
| Cause of the isolated CC 1 value 0 during an untouched host-driven fader sweep | `unresolved` | Not reproduced across 36 representative motor commands in follow-up testing. | — | — |
| Cause of unsolicited Layer A/B expression-pedal CC traffic with no pedal attached | `unresolved` | Observed repeatedly but not explained; no pedal was connected during any of these sessions. | — | — |
| Why passive ALSA receive returns no event, rather than an error, after USB disconnect or power-off | `unresolved` | The behavior is consistently observed and the session policy above accounts for it, but the underlying ALSA/kernel mechanism was not investigated. | — | — |
